...
dim_customers = (
    ...
    .join(
            ...
            == dim_geo.geolocation_zip_code_prefix,
            "left"
    )
    .select(
            "customer_id",
...
df_reviews = spark.read.jdbc(
    POSTGRES_URL, "stg.reviews", properties=POSTGRES_PROPERTIES
)

df_date = (
    df_orders.select(to_date("order_purchase_timestamp").alias("d"))
        .union(df_orders.select(to_date("order_estimated_delivery_date")))
        .union(df_reviews.select(to_date("review_creation_date")))
        .distinct()
)
